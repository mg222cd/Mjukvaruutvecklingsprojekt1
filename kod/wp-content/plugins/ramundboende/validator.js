...
(function($) {
	...
	$.fn.validator = function(options) {
		...
		var errorMessage = {
			...
			monthDayYear : 'The :attribute does not match the format MM/DD/YYYY.',
		...
		var patterns = {
			...
			// MM/DD/YYYY
			monthDayYear : /(0[1-9]|1[012])[- \/.](0[1-9]|[12][0-9]|3[01])[- \/.](19|20)\d\d/,
			...
			password: /((?=.*\d)(?=.*[a-zÃ¥Ã¤Ã¶])(?=.*[A-ZÃ…Ã„Ã–])(?=.*[@#$%]).{8,100})/
		};
		...
		function validate(self){
			...
			if(element === 'input' || element === 'select' || element === 'textarea'){
				...
				if(!self.next().hasClass(settings.errorclass[0])){
					self.after('<div class="'+settings.errorclass.join(' ')+'"></div>');
				}
				errorElement = self.next();

				var label = self.prev().text().toLowerCase();
